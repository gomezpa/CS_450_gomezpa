...
void renderScene(vector<MeshGL>&allMeshes, aiNode *node, glm::mat4 parentMat, GLint modelMatLoc, GLint normMatLoc, glm::mat4 viewMat, int level){
	...
	glm::mat3 normalMat = glm::mat3(glm::transpose(glm::inverse(viewMat * tmpModel)));
	glUniformMatrix3fv(normMatLoc, 1, GL_FALSE, glm::value_ptr(normalMat));
...
void extractMeshData(aiMesh *mesh, Mesh &m) {
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
		...
		AIMeshVertex.normal.x = mesh->mNormals[i].x;
		AIMeshVertex.normal.y = mesh->mNormals[i].y;
		AIMeshVertex.normal.z = mesh->mNormals[i].z;
		
		AIMeshVertex.color = glm::vec4(1.0, 1.0, 0.0, 1.0);
...
int main(int argc, char **argv) {
	...
	GLint lightPosLocation = glGetUniformLocation(programID, "lightPos");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glm::vec4 lightPosView = viewMat * lightPos;
		glUniform4fv(lightPosLocation, 1, glm::value_ptr(lightPosView));
		...
		//	drawMesh(Object[i]);
		//}	
		renderScene(myVector, scene->mRootNode, glm::mat4(1.0), modelMatLoc, normMatLoc, viewMat, 0);
